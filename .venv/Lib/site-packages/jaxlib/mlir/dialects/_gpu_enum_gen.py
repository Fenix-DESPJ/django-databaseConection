...
class BarrierScope(IntEnum):
    ...
    def __str__(self):
        if self is BarrierScope.Subgroup:
            return "subgroup"
        if self is BarrierScope.Workgroup:
            return "workgroup"
        if self is BarrierScope.Cluster:
            return "cluster"
        raise ValueError("Unknown BarrierScope enum entry.")
...
@register_attribute_builder("gpu.GPU_BarrierScopeAttr")
